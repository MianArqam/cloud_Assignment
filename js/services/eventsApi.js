...
const EVENTS_ENDPOINT = 'https://kudago.com/public-api/v1.4/events/?lang=en&page_size=30&location=msk&fields=id,title,description,dates,place,images,site_url';
...
const mapEvent = (event) => ({
  ...
  date: event?.dates?.[0]?.start ? new Date(event.dates[0].start * 1000).toISOString() : new Date().toISOString(),
  ...
  image: event?.images?.[0]?.image || 'https://images.unsplash.com/photo-1523580494863-6f3031224c94?auto=format&fit=crop&w=1200&q=80',
  url: event.site_url || '#'
});
